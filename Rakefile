...
CONFIG = {
  ...
  'news' => File.join(SOURCE, "_posts/news"),
  'post_ext' => "md",
...
desc "Begin a new post in #{CONFIG['news']}"
task :news do
  abort("rake aborted: '#{CONFIG['news']}' directory not found.") unless FileTest.directory?(CONFIG['posts'])
  title = ENV["title"] || "<++>"
  tags = ENV["tags"] || "[]"
  category = ENV["category"] || "news"
  slug = title.downcase.strip.gsub(' ', '-').gsub(/[^\w-]/, '')
  begin
    date = (ENV['date'] ? Time.parse(ENV['date']) : Time.now).strftime('%Y-%m-%d')
    datefull = (ENV['date'] ? Time.parse(ENV['date']) : Time.now).strftime('%Y-%m-%d %H:%M')
  rescue => e
    puts "Error - date format must be YYYY-MM-DD, please check you typed it correctly!"
    exit -1
  end
  filename = File.join(CONFIG['news'], "#{date}-#{slug}.#{CONFIG['post_ext']}")
